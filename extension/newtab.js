const SERVER_URL = 'http://localhost:3742';
...
async function init() {
  ...
  document.addEventListener('keydown', e => {
    if (e.key === 'Escape') { closeAbout(); closeLeaderboard(); closeNamePrompt(); }
  });
  ...
  document.getElementById('about-open-lb').addEventListener('click', () => { closeAbout(); openLeaderboard(); });
  ...
  document.getElementById('name-skip-btn').addEventListener('click', closeNamePrompt);
...
async function openLeaderboard() {
  ...
  try {
    ...
    document.getElementById('lb-game-name').textContent = data.game?.name ?? '—';
    ...
    if (data.scores && data.scores.length) {
      ...
      tbody.innerHTML = '<tr><td colspan="4" class="lb-empty">No scores yet today — be the first</td></tr>';
    }
    ...
    document.getElementById('lb-loading').textContent = 'Could not load scores — is the server running?';
  }
}
